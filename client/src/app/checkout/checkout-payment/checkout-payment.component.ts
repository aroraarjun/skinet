...
export class CheckoutPaymentComponent implements OnInit {
  ...
  async submitOrder() {
    ...
    //since basket can be null so put in try catch
    try {
      //create order is now return promise as we are not subscribing but using firstvaluefrom so use await
  ...
  private async confirmPaymentWithStripe(basket: Basket | null) {
    if (!basket) throw new Error('Baslet is null');
