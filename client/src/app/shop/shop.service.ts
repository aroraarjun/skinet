...
export class ShopService {
  ...
  getProducts(useCache = true) : Observable<Pagination<Product[]>>{
    ...
    // use pipe method so that to do something from response before it goes back to whosubscribed it
    return this.http.get<Pagination<Product[]>>(this.baseUrl + 'products', {params}).pipe(
      map(response => {
        //each time you move to different page new products are appended instead of overwriting
